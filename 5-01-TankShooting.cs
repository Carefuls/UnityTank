...
public class TankShooting : MonoBehaviour
{
    ...
    public Rigidbody m_Shell;                  //声明一个私有 rigidbody 刚体组件 m_Rigidbody
    public Transform m_FireTransform;          //声明一个公共的Transform类对象m_FireTransform，储存炮弹发射的位置以及角度; 
    public Transform m_FireTransform2;         //可以三维发射炮弹所以编写三个
    public Transform m_FireTransform3;
    public Slider m_AimSlider;                 //建立瞄准器        
    ...
    private void Update()
    {
        ...
        else if(Input.GetButton(m_FireButton)&&!m_Fired)        //玩家按下发射键而未松开时
        {
            ...
            m_AimSlider.value = m_CurrentLaunchForce;   //根据上一行代码，控制发射力度显示箭头（也就是瞄准器）逐渐拉长；
        }
