//4-01-TankMovement TotalChange 8
using UnityEngine;
...
public class TankMovement : MonoBehaviour
{
    ...
    public AudioSource m_MovementAudio;    //（4-1）创建一个声音类
    public AudioClip m_EngineIdling;       //（4-2）声明一个公共的 AudioClip 组件，引擎空载声；
    public AudioClip m_EngineDriving;      //（4-3）声明一个公共的 AudioClip 组件，引擎运转声；
    public float m_PitchRange = 0.2f;      //（4-4）设定声音音高初始值
    private float m_OriginalPitch;         //（4-5）存储原始声音音高
    ...
    {
        m_Rigidbody.isKinematic = false;            //关闭刚体m_Rigidbody的isKinematic(isKinematic属性是确定刚体是否接受动力学模拟，此影响不仅包括重力感应，还包括速度、阻力、质量等的物理模拟)，即刚体m_Rigidbody将受物理的影响；
    ...
    private void Start()
    {
        ...
        m_OriginalPitch = m_MovementAudio.pitch;        //（4-6）获取音频源的音高；
    }
