//5-02-TankManager  TotalChange 5
using System;
using UnityEngine;
...
public class TankManager {
    ...
    private TankShooting m_Shooting; //（5-01），声明一个私有TankShooting脚本组件m_Shooting;


    //坦克的初始化设置
    public void Setup() {
        m_Movement = m_Instance.GetComponent<TankMovement>(); //将坦克的TankMovement脚本组件返回给 m_Movement
        m_Shooting = m_Instance.GetComponent<TankShooting>(); //（5-02），将坦克的TankShooting脚本组件返回给 m_Shooting；
        m_Movement.m_PlayerNumber = m_PlayerNumber; //给脚本组件设置玩家编号
        m_Shooting.m_PlayerNumber = m_PlayerNumber; //（5-03），给脚本组件设置玩家编号；
    ...
    public void DisableControl() {
        ...
        m_Shooting.enabled = false;              //（5-04）停用m_Shooting脚本；
    ...
    public void EnableControl() {
        ...
        m_Shooting.enabled = true;              //（5-05），启用m_Shooting脚本；
